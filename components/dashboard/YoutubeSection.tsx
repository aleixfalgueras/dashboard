...
import {YoutubeConsistencyMetrics} from "@/lib/types/youtube-types";
...
function PostingHeatmap({ data }: { data: YoutubeConsistencyMetrics['heatmapData'] }) {
  const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
  const hours = ['12am', '3am', '6am', '9am', '12pm', '3pm', '6pm', '9pm']
  
  // Find max value for color scaling
  const maxCount = Math.max(...data.map(cell => cell.videoCount), 1)
  
  // Get color intensity based on video count
        ...
        <div className="inline-block">
          ...
          <div className="flex items-center justify-center gap-3 mt-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-muted-foreground">Few videos</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: 'hsl(var(--accent) / 1)' }} />
              <span className="text-xs text-muted-foreground">Many videos</span>
            </div>
          </div>
        </div>
      </div>
